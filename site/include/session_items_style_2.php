    	...
    	<?php
			...
			if (!empty($conferenceSessions)) {
				foreach ($conferenceSessions as $item) {
					...
					$token = rand(); ?>
    				<div class="session-item shadow-border" style="background-color: <?php echo !empty($item->color)?$item->color:""; ?>"  onclick="showDetails('<?php echo $token ?>');" >
        				<?php if ($item->date!=$date && $item->start_time!=$time) { ?>
        					<div class="date-box">
    							<div class="box-date"><?php echo JBusinessUtil::getShortWeekDate($item->date)?> </div>
    							<?php $date=$item->date; ?>  
	    					</div>
    					<?php } ?>
