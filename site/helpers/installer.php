...
/**
 * JBD Installer helper class
 *
 * @since  3.1
 */
abstract class JBDInstallerHelper {
	/**
	 * Downloads a package
	 *
	...
	public static function downloadPackage($url, $target = false) {
		...
		$tmpPath = JBusinessUtil::getSiteConfig()->tmp_path;
	...
	public static function unpack($p_filename, $alwaysReturnArray = false) {
		...
		try {
			$archive = new Archive(array('tmp_path' => JBusinessUtil::getSiteConfig()->tmp_path));
	...
	public static function detectType($p_dir) {
		...
		foreach ($files as $file) {
			...
			if ($xml->getName() !== 'extension' && $xml->getName() !== 'application') {
				unset($xml);
				continue;
			}
	...
	public static function cleanupInstall($package, $resultdir) {
		...
		} elseif (is_file(\JPath::clean($config->tmp_path . '/' . $package))) {
			// It might also be just a base filename
			\JFile::delete(\JPath::clean($config->tmp_path . '/' . $package));
		}
	}
