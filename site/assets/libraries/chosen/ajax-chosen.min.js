(function($){return $.fn.ajaxChosen=function(settings,callback,chosenOptions){var chosenXhr,defaultOptions,options,select,chosenSelector;var chosenSelector='.chzn-container';if(!$(chosenSelector).length){chosenSelector='.chosen-container'}
    if(settings==null){settings={}}
    if(chosenOptions==null){chosenOptions={}}
    defaultOptions={minTermLength:3,afterTypeDelay:500,jsonTermKey:"term",keepTypingMsg:"Keep typing...",lookingForMsg:"Looking for"};select=this;chosenXhr=null;options=$.extend({},defaultOptions,$(select).data(),settings);this.chosen(chosenOptions?chosenOptions:{});return this.each(function(){return $(this).next(chosenSelector).find(".search-field > input, .chzn-search > input, .chosen-search > input").bind('keyup',function(){var field,msg,success,untrimmed_val,val;untrimmed_val=$(this).val();val=$.trim($(this).val());msg=val.length<options.minTermLength?options.keepTypingMsg:options.lookingForMsg+(" '"+val+"'");select.next(chosenSelector).find('.no-results').text(msg);if(val===$(this).data('prevVal')){return!1}
        ...
        success=options.success;options.success=function(data){var items,nbItems,selected_values;if(data==null){return}
            selected_values=[];select.find('option').each(function(){if(!$(this).is(":selected")){return $(this).remove()}else{return selected_values.push($(this).val()+"-"+$(this).text())}});select.find('optgroup:empty').each(function(){return $(this).remove()});items=callback!=null?callback(data,field):data;nbItems=0;$.each(items,function(i,element){var group,text,value;nbItems++;if(element.group){group=select.find("optgroup[label='"+element.text+"']");if(!group.size()){group=$("<optgroup />")}
                ...
                if($.inArray(value+"-"+text,selected_values)===-1){return $("<option />").attr('value',value).html(text).appendTo(select)}}});if(nbItems){select.trigger("liszt:updated");select.trigger("chosen:updated")}else{select.data().chosen.no_results_clear();select.data().chosen.no_results(field.val())}
