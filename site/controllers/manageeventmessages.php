...
require_once(JPATH_COMPONENT_ADMINISTRATOR.DS.'controllers'.DS.'eventmessages.php');
require_once(JPATH_COMPONENT_ADMINISTRATOR.DS.'models'.DS.'eventmessages.php');

class JBusinessDirectoryControllerManageEventMessages extends JBusinessDirectoryControllerEventMessages
{
    /**
    ...
    public function delete()
    {
        ...
        $query = $db->getQuery(true)
            ->delete($db->quoteName('#__jbusinessdirectory_event_messages'))
            ->where('id = '. (int) $cid);

        $db->setQuery($query);
        $result = $db->execute();

        $this->setRedirect('index.php?option=com_jbusinessdirectory&view=manageeventmessages');
    }
}