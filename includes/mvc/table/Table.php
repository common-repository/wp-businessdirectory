 ...
 *
 * @since  11.1
 * @tutorial  JBD.Platform/jtable.cls
 */
abstract class Table extends \JObject 
{
	/**
	...
	public static function getInstance($type, $prefix = 'JTable', $config = array())
	{
		...
		if (!class_exists($tableClass))
		{
			...
			$paths = self::addIncludePath($path);
			
			$pathIndex = 0;
			while (!class_exists($tableClass) && $pathIndex < count($paths))
			{
				$tryThis = \JPath::find($paths[$pathIndex], strtolower($type) . '.php');
