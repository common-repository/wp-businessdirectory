...
abstract class JLoader {
	...
	public static function registerAlias($alias, $original, $version = false) {
		...
		if (! isset ( self::$classAliases [$lowercasedAlias] )) {
			...
			if (! isset ( self::$classAliasesInverse [$original] )) {
				self::$classAliasesInverse [$original] = array (
						$lowercasedAlias 
				);
			} else {
	 ...
	 * @return void
	 *
	 * @throws RuntimeException @note The default argument of $type will be changed in J4 to be 'psr4'
	 * @since 12.3
	 */
	public static function registerNamespace($namespace, $path, $reset = false, $prepend = false, $type = 'psr0') {
		...
		if ($reset || ! isset ( self::$namespaces [$type] [$namespace] )) {
			self::$namespaces [$type] [$namespace] = array (
					$path 
			);
		}		
	...
	public static function loadByPsr4($class) {
		...
		//dump("Class path: ".$classPath);
		$classPathParts = explode("/",$classPath);
		if(count($classPathParts)==3){
			$classPathParts[1]=strtolower($classPathParts[1]);
	 ...
	 *
	 * @param string $class
	 *        	The class to be loaded (without prefix).
	 * @param array $lookup
	...
	private static function _load($class, $lookup) {
		...
		foreach ( $lookup as $base ) {
			...
			if ($partsCount === 1) {
				// Generate the path based on the class name parts.
				$path = $base . '/' . implode ( '/', array_map ( 'strtolower', array (
						$parts [0],
						$parts [0] 
				) ) ) . '.php';
