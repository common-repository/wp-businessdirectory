...
use MVC\Application\ApplicationHelper;
use MVC\Component\ComponentHelper;
...
class FileLayout extends BaseLayout
{
	...
	protected function getPath()
	{
		...
		if (!$layoutId)
		{
			...
			var_dump("<strong>There is no active layout</strong>");
			//var_dump($this->layoutId);
			return;
		}
		...
		if (!$includePaths)
		{
			...
			var_dump('<strong>There are no folders to search for layouts:</strong> ' . $layoutId);
			return;
		}
		...
		//var_dump('<strong>Include Paths:</strong> ' . print_r($includePaths, true));
		...
		if (!$foundLayout)
		{
			...
			var_dump('<strong>Unable to find layout: </strong> ' . $layoutId);
			return;
		}
	...
	public function getDefaultIncludePaths()
	{
		...
		//$paths[] = JPATH_THEMES . '/' .  . '/html/layouts';
