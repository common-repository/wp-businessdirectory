...
\JLoader::import('environment.browser');
\JLoader::import('filesystem.file');
//\JLoader::import('filesystem.path');
...
abstract class HTMLHelper
{
	...
	public static function _($key)
	{
		...
		self::addIncludePath(__DIR__.'/html/');
	...
	public static function stylesheet($file, $options = array(), $attribs = array())
	{
		...
		if (!is_array($attribs))
		{
			//var_dump('The stylesheet method signature used has changed, use (file, options, attributes) instead');
...
//			$document->addStyleSheet($include, $options, $attribs);
	...
	public static function calendar($value, $name, $id, $format = '%Y-%m-%d', $attribs = array())
	{
		...
		$helperPath = 'date/date-helper.min.js';

		if (!empty($calendar) && is_dir(JPATH_ROOT . '/media/system/js/fields/calendar-locales/date/' . strtolower($calendar)))
		{
			$helperPath = 'date/' . strtolower($calendar) . '/date-helper.min.js';
		}

		// Get the appropriate locale file for the current language
		$localesPath = 'date/locales/en.js';
		...
		return LayoutHelper::render('form.field.calendar', $data, null, null);
	}
	 ...
	 *
	 * @since   3.0
	 * @deprecated  4.0 Use `json_encode()` or `MVC\Registry\Registry::toString('json')` instead
	 */
