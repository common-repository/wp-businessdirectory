				...
				<li>
					<?php if(isset($category["subCategories"]) && count($category["subCategories"]) > 0) { 
						...
						<a aria-expanded="true" href="#">
							...
							<?php } ?>
							<span onclick="goToLink('<?php echo JBusinessUtil::getEventCategoryLink($category[0]->id, $category[0]->alias) ?>')"> <?php echo $category[0]->name; ?></span> 
						...
						<ul aria-expanded="false" class="collapse">
							...
							if($nrCategories>0) {
								foreach($category["subCategories"] as $subcategory) {
									...
									?>
									<li>
										<a href="<?php echo JBusinessUtil::getEventCategoryLink($subcategory[0]->id, $subcategory[0]->alias) ?>">
					...
					<?php } else { ?>
						<a href="<?php echo JBusinessUtil::getEventCategoryLink($category[0]->id, $category[0]->alias) ?>">
						<?php if(!empty($category[0]->icon)) { ?>
							<span class="la la-<?php echo $category[0]->icon ?>"></span> 
						<?php } ?>
