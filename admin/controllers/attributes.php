...
class JBusinessDirectoryControllerAttributes extends JControllerAdmin {
	/**
	 * Class constructor.
	 *
	 ...
	 * @since   1.6
	 */
	public function getModel($name = 'Attribute', $prefix = 'JBusinessDirectoryModel', $config = array('ignore_request' => true)) {
	...
	public function delete() {
		...
		if (!is_array($cid) || count($cid) < 1) {
			JFactory::getApplication()->enqueueMessage(JText::_('COM_JBUSINESSDIRECTORY_NO_ATTRIBUTE_SELECTED'), 'error');
		} else {
			// Get the model.
			...
			if (is_array($result)) {
				// Remove the items.
				if (!empty($result)) {
					JFactory::getApplication()->enqueueMessage(JText::plural('COM_JBUSINESS_DIRECTORY_S_ATTRIBUTES_IN_USE', implode(',', $result)), 'error');
//				$this->setMessage($model->getError());
					...
					$this->setMessage(JText::plural('COM_JBUSINESS_DIRECTORY_N_ATTRIBUTES_DELETED', count($cid)));
				}
			} else {
				// Remove the items.
				if (!$result) {
		...
		$this->setRedirect('index.php?option=com_jbusinessdirectory&view=attributes');
	}
