...
class JBusinessDirectoryControllerEmailTemplates extends JControllerAdmin {
	/**
	 * Class constructor.
	 *
	 ...
	 * @since   1.6
	 */
	public function getModel($name = 'EmailTemplate', $prefix = 'JBusinessDirectoryModel', $config = array('ignore_request' => true)) {
	...
	public function delete() {
		...
		if (!is_array($cid) || count($cid) < 1) {
			JFactory::getApplication()->enqueueMessage(JText::_('COM_JBUSINESSDIRECTORY_NO_EMAIL_SELECTED'), 'error');
		} else {
			// Get the model.
			...
			if (!$model->delete($cid)) {
				...
				$this->setMessage(JText::plural('COM_JBUSINESS_DIRECTORY_N_EMAIL_TEMPLATES_DELETED', count($cid)));
			}
		}

		$this->setRedirect('index.php?option=com_jbusinessdirectory&view=emailtemplates');
	}
