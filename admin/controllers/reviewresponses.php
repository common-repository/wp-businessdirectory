...
class JBusinessDirectoryControllerReviewresponses extends JControllerLegacy {
	...
	public function save($key = null, $urlVar = null) {
		...
		if ($model->store($data)) {
			...
			$msg = '';
			JFactory::getApplication()->enqueueMessage(JText::_("LNG_ERROR_SAVING_REVIEW_RESPONSE"), 'error');
			$this->setRedirect('index.php?option=com_jbusinessdirectory&controller=reviewresponses&view=reviewresponses', $msg);
		}
	...
	public function delete() {
		...
		if (!is_array($cid) || count($cid) < 1) {
			JFactory::getApplication()->enqueueMessage(JText::_("COM_JBUSINESSDIRECTORY_NO_REVIEW_RESPONSE_SELECTED"), 'error');
		} else {
			// Get the model.
			$model = $this->getModel("Reviewresponse");
	...
	public function changeState() {
		...
		$model = $this->getModel('Reviewresponse');
		if (!$model->changeState($cid[0], $value)) {
			$this->setMessage(JText::_('LNG_ERROR_CHANGE_STATE'), 'warning');
			$error = true;
		} else {
