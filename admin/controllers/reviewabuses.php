...
class JBusinessDirectoryControllerReviewabuses extends JControllerLegacy {
	...
	public function save($key = null, $urlVar = null) {
		...
		if ($model->store($data)) {
			...
			$msg = '';
			JFactory::getApplication()->enqueueMessage(JText::_('LNG_ERROR_SAVING_REVIEW_ABUSE'), 'error');
			$this->setRedirect('index.php?option=com_jbusinessdirectory&controller=reviewabuses&view=reviewabuses', $msg);
		}
	...
	public function delete() {
		...
		if (!is_array($cid) || count($cid) < 1) {
			JFactory::getApplication()->enqueueMessage(JText::_('COM_JBUSINESSDIRECTORY_NO_REVIEW_ABUSE_SELECTED'), 'error');
		} else {
			// Get the model.
			$model = $this->getModel("Reviewabuse");
