...
window.addEventListener('load', function() {
    JBD.submitbutton = function(task) {
        if (task != 'reviewquestions.delete' || confirm('<?php echo JText::_('LNG_ARE_YOU_SURE_YOU_WANT_TO_DELETE', true);?>')) {
            JBD.submitform(task);
        }
    }
});
...
<div id="jbd-container" class="jbd-container">
    <form action="<?php echo JRoute::_('index.php?option=com_jbusinessdirectory&view=reviewquestions');?>" method="post" name="adminForm" id="adminForm">
        ...
        <?php } else { ?>
            <table class="jtable"  id="itemList">
                ...
                <tr class="jtable-head-row">
                    ...
                    <th class="jtable-head-row-data">
                        <?php echo HTMLHelper::_('searchtools.sort', 'LNG_TYPE', 'ct.type', $listDirn, $listOrder); ?>
                    </th>
                    <th class="jtable-head-row-data" style="text-align: center">
                        <?php echo HTMLHelper::_('searchtools.sort', 'JSTATUS', 'ct.published', $listDirn, $listOrder); ?>
                    </th>
                ...
                <?php foreach ($this->items as $index => $item) :
                    ...
                    <tr class="jtable-body-row" data-draggable-group="3">
                        ...
                        <td class="jtable-body-row-data">
                            <?php if ($canEdit) : ?>
                                <a href="<?php echo JRoute::_('index.php?option=com_jbusinessdirectory&task=reviewquestion.edit&id='.$item->id);?>">
                          ...
                          <td class="jtable-body-row-data">
                            <?php echo $this->types[$item->type]->text ?>
                        </td>
                        <td class="jtable-body-row-data">
                            <?php echo HTMLHelper::_('jbdgrid.published', $item->published, $i, 'reviewquestions.', true, 'cb', true, true, $item->id); ?>
                        </td>
                        <td class="jtable-body-row-data">
