 ...
 */
class JBusinessDirectoryModelOfferMessages extends JModelList
{
     ...
     * @since   1.6
     */
    protected function populateState($ordering = 'cm.id', $direction = 'asc')
    {
    ...
    protected function getListQuery()
    {
        ...
        $query->from($db->quoteName('#__jbusinessdirectory_offer_messages') . ' AS cm');

        $query->select('bc.subject as offerName');
        $query->leftJoin($db->quoteName('#__jbusinessdirectory_company_offers') . ' AS bc ON bc.id=cm.offer_id');
        ...
        if (!empty($search)) {
            if ($typeId == FILTER_OFFER_NAME) {
                $query->where("bc.subject LIKE '%" . trim($db->escape($search)) . "%'");
                ...
                $query->where("bc.subject LIKE '%" . $val . "%' OR cm.name LIKE '%" . $val . "%' OR cm.surname LIKE '%" . $val . "%' OR cm.email LIKE '%" . $val . "%'");
            }
        }
    ...
    function getSearchTypes()
    {
        $states = array();
        $state = new stdClass();
        $state->value = FILTER_OFFER_NAME;
        $state->text = JTEXT::_("LNG_MESSAGE_OFFER_NAME");
        $states[] = $state;
        $state = new stdClass();
